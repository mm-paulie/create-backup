...
async function run(obj) {
    ...
    let browser = await puppeteer.launch({headless:true,  executablePath: revisionInfo.executablePath, defaultViewport:{
            width: 1920,
            height: 1080
        } ,args:[
            '--start-maximized'
    ...
    //get rid of title
    await page.addStyleTag({content: '.mdl-card__actions{display: none}'})

    if(obj.addBorder) {
        await page.addStyleTag({content: 'iframe{box-sizing: border-box; border: 1px solid black}'})
        await page.addStyleTag({content: '.banner{border: 0px solid black}'})
    }
    ...
    for (const frame of page.mainFrame().childFrames()){
        ...
        let pad = box.content[0].y < 100 ? -1 : 0;
        ...
        await page.screenshot({
            ...
            type: 'jpeg',
            clip: {x: obj.addBorder ? box.content[0].x-1 : box.content[0].x , y: obj.addBorder ? box.content[0].y + pad : box.content[0].y+1+ pad, width: box.width, height: box.height}
        });
